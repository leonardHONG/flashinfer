...
// 300s @ 2GHz upper bound before a waiting kernel traps (deadlock surfacing).
#ifndef FLASHINFER_SM90_PUSH_DISABLE_TIMEOUT
#define FLASHINFER_SM90_PUSH_CHECK_TIMEOUT(s) \
  ((clock64() - (s)) > (300ll * 2000ll * 1000ll * 1000ll))
#else
...
__device__ __forceinline__ int32_t wait_tag_u64(uint64_t const* addr, uint32_t tag) {
  auto s = clock64();
  uint64_t v = ld_acquire_sys_u64(addr);
  while (static_cast<uint32_t>(v >> 32) != tag) {
    __nanosleep(64);
