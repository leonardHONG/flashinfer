...
# Grouped SM90 FP8 block-scale MoE GEMM (`moe_gemm` binding) vs a pure-torch
# oracle. Layout contract (csrc/fp8_blockscale_gemm_sm90_binding.cu):
...
def test_moe_gemm_grouped_fp8(runner, G, N, K, m_per, probe):
    ...
    sz = runner.get_moe_workspace_size(max(M, 1), N, K, 1, G, True, True)
...
@pytest.mark.parametrize("G,N,K,m_per,probe", CASES)
def test_moe_gemm_grouped_bf16_internal_quant(runner, G, N, K, m_per, probe):
    """BF16 x BF16 internal-quantization path, judged vs an unquantized oracle
    at fp8 quant-noise tolerance."""
    torch.manual_seed(0)
    M = sum(m_per)
    dev = "cuda"
    offsets = torch.tensor(
    ...
    a = torch.randn(max(M, 1), K, device=dev, dtype=torch.bfloat16)[:M]
    w = torch.randn(G, N, K, device=dev, dtype=torch.bfloat16)
    out = torch.empty(M, N, device=dev, dtype=torch.bfloat16)

    sz = runner.get_moe_workspace_size(max(M, 1), N, K, 1, G, False, False)
    runner.configure_workspace(
        torch.empty(max(int(sz), 1), device=dev, dtype=torch.uint8)
    )
    try:
        runner.moe_gemm(out, a, w, offsets, N, K, None, None, False)
        torch.cuda.synchronize()
    except Exception as e:  # noqa: BLE001
        if probe is not None:
            pytest.xfail(f"probe [{probe}] unsupported: {e!r}")
    ...
    for e in range(G):
        s0, s1 = offsets[e].item(), offsets[e + 1].item()
        if s1 > s0:
            oracle[s0:s1] = a[s0:s1].float() @ w[e].float().T
    ...
    ok = float(err_rms / nrm) < 0.06 and float(cos) > 0.99
    if not ok and probe is not None:
        pytest.xfail(
            f"probe [{probe}] numerically off: err/rms={float(err_rms / nrm):.4f}"
        )
