...
__global__ void wait_acks_kernel(PushLayout L, const int32_t* __restrict__ round_ctr) {
  ...
  uint32_t want = static_cast<uint32_t>(*round_ctr) - 1u;
  wait_tag_u64(L.ack_cell(L.rank, d), want);
}

template <int TOP_K>
__global__ void count_kernel(const int32_t* __restrict__ topk_ids, int num_local_experts,
                             int num_experts_total, int32_t* __restrict__ lc,
                             int32_t* __restrict__ loff, int local_num_tokens) {
...
template <int TOP_K>
__global__ void count_dedup_kernel(const int32_t* __restrict__ topk_ids, int num_local_experts,
                                   int num_experts_total, int32_t* __restrict__ lc,
                                   int32_t* __restrict__ loff, int32_t* __restrict__ pc,
                                   int32_t* __restrict__ ploff, int local_num_tokens) {
...
__global__ void wait_prefix_kernel(
  ...
  for (int i = threadIdx.x; i < nkeys; i += blockDim.x) {
    int c = wait_tag_u64(cells + i, tag);
  ...
  for (int s = threadIdx.x; s < eps; s += blockDim.x) {
    sbase[s] = wait_tag_u64(L.base_cell(L.rank, s), tag);
  }
  __syncthreads();
  if (threadIdx.x == 0) {
    int64_t acc = 0;
...
__global__ void wait_combine_kernel(PushLayout L, const int32_t* __restrict__ round_ctr) {
  ...
  wait_tag_u64(L.cdone_cell(L.rank, s), static_cast<uint32_t>(*round_ctr));
}
...
#define DISPATCH_CASE(K)                                                                           \
  case K: {                                                                                        \
    if (nroutes > 0) {                                                                             \
      count_kernel<K><<<(nroutes + 255) / 256, blk, 0, stream>>>(ids, E, E_total, lcp, loffp, lt); \
    }                                                                                              \
...
#define DISPATCH_DEDUP_CASE(K)                                                                    \
  case K: {                                                                                       \
    if (lt > 0) {                                                                                 \
      count_dedup_kernel<K>                                                                       \
          <<<(lt + 255) / 256, blk, 0, stream>>>(ids, E, E_total, lcp, loffp, pcp, ploffp, lt);   \
    }                                                                                             \
