...
struct GroupedGemmProblemVisitor {
  ...
  static __device__ int tile_m_idx() { return blockIdx.x; }

  static __device__ int tile_n_idx() { return blockIdx.y; }

  static __device__ int problem_idx() { return blockIdx.z; }
...
struct PlainGemmProblemVisitor {
  ...
  static __device__ int problem_idx() { return 0; }

  static __device__ int m_offset(Input const& input) { return 0; }

  static __device__ int n_offset(Input const& input) { return 0; }

  static __device__ int m_boundary(Input const& input) { return input.shape_m; }
};
...
template <typename ProblemVisitor, typename ElementA, typename ElementB, typename ElementD,
          Layout LayoutD, typename WGMMA_OP, int TILE_M, int TILE_N, int TILE_K, int NUM_STAGES,
          bool IsPersistentKernel = false>
__global__ void __launch_bounds__(TILE_M == 64 ? 256 : 384, 1)
    cooperative_1x128_by_128x128_fp8_gemm_kernel(
        ElementD* gmem_d, int ld_d, float const* scales_b,
        typename ProblemVisitor::Input problem_input, int shape_n, int shape_k,
  ...
  for (int i = 0; i < NUM_STAGES; i++) {
    ...
    smem_b[i] = reinterpret_cast<ElementB*>(smem_buffer + NUM_STAGES * SMEM_A_SIZE_PER_STAGE +
                                            i * SMEM_B_SIZE_PER_STAGE);
    smem_scales_a[i] = reinterpret_cast<float*>(
        smem_buffer + NUM_STAGES * (SMEM_A_SIZE_PER_STAGE + SMEM_B_SIZE_PER_STAGE) +
        i * SMEM_SCALES_A_SIZE_PER_STAGE);
    full_bars[i] =
        reinterpret_cast<Barrier*>(smem_buffer +
                                   NUM_STAGES * (SMEM_A_SIZE_PER_STAGE + SMEM_B_SIZE_PER_STAGE +
  ...
  if constexpr (ScaleGranKB != 0) {
    int end_index = !IS_UNIFORM_SCALE_B && iters_in_former_scales_b < TILE_N / 8 ? scales_b_ld * 2
                                                                                 : scales_b_ld;
#pragma unroll
      ...
      for (int k_iter = 0; k_iter < div_up(shape_k, K_PER_ITER); k_iter++) {
        auto copy_func = [&](Barrier& empty_bar, int stage_range_start, int stage_range_end) {
          ...
          for (int i = stage_range_start; i < stage_range_end; i++) {
            ...
            if constexpr (std::is_same_v<ProblemVisitor,
                                         StridedBatchedGemmProblemVisitor<TILE_M, TILE_N>>) {
              int scale_y_offset = problem_idx * (problem_input.stride_scales_a /
                                                  (div_up(problem_input.shape_m, 4) * 4));
              // The scales has been aligned to 16 bytes
              cde::cp_async_bulk_tensor_2d_global_to_shared(
                  smem_scales_a[i], &tensor_map_scales_a, (tile_m_idx * TILE_M) / ScaleGranMA,
                  scale_y_offset + k_idx / ScaleGranKA, bar);
            } else {
              // The scales has been aligned to 16 bytes
              cde::cp_async_bulk_tensor_2d_global_to_shared(
                  smem_scales_a[i], &tensor_map_scales_a,
                  (problem_m_padded_offset + tile_m_idx * TILE_M) / ScaleGranMA,
                  k_idx / ScaleGranKA, bar);
            }
          }
          for (int i = stage_range_start; i < stage_range_end; i++) {
...
#pragma unroll
    ...
    if constexpr (LayoutD == Layout::RowMajor) {
      ...
      if constexpr (std::is_same_v<ProblemVisitor,
                                   StridedBatchedGemmProblemVisitor<TILE_M, TILE_N>>) {
        gmem_d_this_block =
            gmem_d + problem_idx * problem_input.stride_d + (tile_m_idx * TILE_M) * ld_d;
      } else {
      ...
      int int4_per_line =
          is_last_tile_n ? int4_per_global_line % int4_per_tile_line : int4_per_tile_line;
      ...
      for (int line_idx = warp_idx; line_idx < num_lines; line_idx += num_warps) {
        ...
        for (int elem_idx = lane_idx; elem_idx < int4_per_line; elem_idx += 32) {
          int4* g_data_addr =
              reinterpret_cast<int4*>(&gmem_d_this_block[line_idx * ld_d + tile_n_idx * TILE_N]) +
              elem_idx;
          int4* s_data_addr =
              &smem_c_int4[line_idx * (int4_per_tile_line +
                                       SMEM_C_PADDING * sizeof(ElementD) / sizeof(int4)) +
                           elem_idx];
...
template <typename ElementA, Layout LayoutA, typename ElementB, Layout LayoutB, typename ElementD,
          Layout LayoutD, typename ElementAccumulator, typename ElementCompute,
          typename ElementScalar, int TILE_M, int TILE_N, int TILE_K, ScaleType ScaleTypeA,
          ScaleType ScaleTypeB, int ScaleGranMA = 0, int ScaleGranKA = 0, int ScaleGranNB = 0,
...
class Fp8Gemm {
 ...
 public:
  static constexpr int get_smem_size(int num_stages, int max_shape_k = MAX_SHAPE_K) {
    auto smem_size = num_stages * (SMEM_A_SIZE_PER_STAGE + SMEM_B_SIZE_PER_STAGE + sizeof(Barrier) +
                                   sizeof(Barrier));

    if constexpr (ScaleTypeA == ScaleType::PerSubChannel) {
      auto scale_smem_size = num_stages * div_up(TILE_M, ScaleGranMA) *
                             div_up(TILE_K, ScaleGranKA) * sizeof(ElementScalar);
 ...
 private:
  static constexpr int get_num_stages() {
    ...
    static_assert(get_smem_size(4) <= sm90_capacity,
                  "The required shared memory size is too large");
    return 4;
  }
 ...
 public:
  Fp8Gemm() {
    static_assert(
        !(ScaleTypeA == ScaleType::PerSubChannel && (ScaleGranMA == 0 || ScaleGranKA == 0)));
  ...
  static void run(ElementA* gmem_a, ElementB* gmem_b, ElementD* gmem_d, ElementScalar* scales_a,
                  ElementScalar const* scales_b, int num_problems, int64_t const* problem_m_offsets,
                  int shape_n, int shape_k, int max_shape_m, cudaStream_t stream = 0,
...
#define Kernel                                                                               \
  cooperative_1x128_by_128x128_fp8_gemm_kernel<ProblemVisitor, ElementA, ElementB, ElementD, \
                                               LayoutD, WGMMA_OP, TILE_M, TILE_N, TILE_K,    \
                                               NUM_STAGES, true>
    ...
    Kernel<<<grid_size, BLOCK_SIZE, smem_size, stream>>>(gmem_d, shape_n, scales_b, problem_input,
                                                         shape_n, shape_k, tma_a_desc, tma_b_desc,
                                                         tma_scales_a_desc, guessed_m);
#undef Kernel
  }

  // PlainGemm
  static void run(ElementA* gmem_a, int ld_a, ElementB* gmem_b, int ld_b, ElementD* gmem_d,
                  int ld_d, ElementScalar* scales_a, ElementScalar const* scales_b, int shape_m,
                  int shape_n, int shape_k, cudaStream_t stream = 0, int guessed_m = TILE_M) {
...
#define Kernel                                                                               \
  cooperative_1x128_by_128x128_fp8_gemm_kernel<ProblemVisitor, ElementA, ElementB, ElementD, \
    ...
    Kernel<<<grid_size, BLOCK_SIZE, smem_size, stream>>>(gmem_d, ld_d, scales_b, problem_input,
                                                         shape_n, shape_k, tma_a_desc, tma_b_desc,
                                                         tma_scales_a_desc, guessed_m);
#undef Kernel
  }
  ...
  // StridedBatchedGemm
  static void run(ElementA* gmem_a, int ld_a, int stride_a, ElementB* gmem_b, int ld_b,
                  int stride_b, ElementD* gmem_d, int ld_d, int stride_d, ElementScalar* scales_a,
                  int stride_scales_a, ElementScalar const* scales_b, int shape_m, int shape_n,
                  int shape_k, int num_problems, cudaStream_t stream = 0) {
...
#define Kernel                                                                               \
  cooperative_1x128_by_128x128_fp8_gemm_kernel<ProblemVisitor, ElementA, ElementB, ElementD, \
    ...
    assert(shape_n % TILE_N == 0);
    auto tma_a_desc =
        make_2d_tma_a_desc(gmem_a, shape_m * num_problems, shape_k, ld_a * sizeof(*gmem_a));
    auto tma_b_desc =
        make_2d_tma_b_desc(gmem_b, shape_k, shape_n * num_problems, ld_b * sizeof(*gmem_b));
    auto tma_scales_a_desc = make_2d_tma_scales_a_desc(scales_a, shape_m, shape_k, num_problems);
    static_assert(TILE_N == WGMMA_OP::N);
    typename ProblemVisitor::Input problem_input{shape_m,  ld_a,     stride_a,       ld_b,
                                                 stride_b, stride_d, stride_scales_a};
...
#undef Kernel
  ...
  template <typename T>
  static CUtensorMap make_2d_tma_a_desc(T* global_address, uint64_t gmem_rows, uint64_t gmem_cols,
                                        uint64_t global_stride_in_bytes = 0) {
    return make_2d_tma_desc(global_address, LayoutA, gmem_rows, gmem_cols, global_stride_in_bytes,
                            TILE_M, TILE_K);
  }

  template <typename T>
  static CUtensorMap make_2d_tma_b_desc(T* global_address, uint64_t gmem_rows, uint64_t gmem_cols,
                                        uint64_t global_stride_in_bytes = 0) {
    return make_2d_tma_desc(global_address, LayoutB, gmem_rows, gmem_cols, global_stride_in_bytes,
                            TILE_K, TILE_N);
  }

  template <typename T>
  static CUtensorMap make_2d_tma_scales_a_desc(T* global_address, uint64_t shape_m,
                                               uint64_t shape_k, int num_problems = 1,
                                               uint64_t global_stride_in_bytes = 0) {
    ...
    return make_2d_tma_desc(global_address, Layout::ColMajor, div_up(shape_m, ScaleGranMA),
                            div_up(shape_k, ScaleGranKA) * num_problems, global_stride_in_bytes,
                            TILE_M / ScaleGranMA, TILE_K / ScaleGranKA,
                            CUtensorMapSwizzle::CU_TENSOR_MAP_SWIZZLE_NONE);
  }

  template <typename T>
  static CUtensorMap make_2d_tma_desc(
      T* global_address, Layout layout, uint64_t gmem_rows, uint64_t gmem_cols,
...
__global__ void scale_1x128_kernel(OutputType* output, ScaleType* scales,
  ...
  using Input2Type =
      typename std::conditional<std::is_same<InputType, half>::value, half2, __nv_bfloat162>::type;
...
#pragma unroll
    for (int i = 0; i < 2; i++) {
      if (scales_idx_x * 128 + i * 64 + lane_id >= dim_x) {
        ...
        input_amax = InputType(
            __hmax(input_amax, __hmax(__habs(input_frag2[i].x), __habs(input_frag2[i].y))));
      }
    }
...
__global__ void scale_1x128_kernel(OutputType* output, float* scales, InputType const* input,
                                   int64_t const* problem_m_offsets, int num_problems, int dim_x,
  ...
  for (size_t warp_idx = (threadIdx.x + blockIdx.x * blockDim.x) / 32; warp_idx < total_scales;
    ...
    kernel_utils::fast_divmod(scales_idx_y, scales_idx_x, warp_idx, scales_along_dim_x,
    ...
    if constexpr (UseBinarySearch) {
      ...
      if (scales_idx_y >= boundary_left) {
        ...
        padded_offset =
            deep_gemm::compute_padded_offset(boundary_left, problem_idx + 1) - boundary_left;
...
template <typename InputType, typename OutputType, typename ScaleType = float>
__global__ void scale_1x128_reshape_kernel(OutputType* output, ScaleType* scales,
                                           InputType const* const input, int dim_x, int dim_h,
                                           int dim_y, int stride_x) {
#if (defined(__CUDA_ARCH__) && (__CUDA_ARCH__ >= 890))
  size_t scales_along_dim_x = div_up(dim_x, 128);
  size_t scales_along_dim_y = div_up(dim_y, 1);
  ...
  for (size_t warp_idx = (blockIdx.x * blockDim.x + threadIdx.x) / 32;
    ...
    InputType const* input_line = input + (size_t)scales_idx_y * stride_x * dim_h +
                                  (size_t)scales_idx_h * stride_x + scales_idx_x * 128;
    InputType input_amax = InputType(0);
    int lane_id = threadIdx.x % 32 * 2;
...
#pragma unroll
    ...
    OutputType* output_line = output + (size_t)scales_idx_h * dim_y * dim_x +
                              (size_t)scales_idx_y * dim_x + scales_idx_x * 128;
#pragma unroll
    for (int i = 0; i < 2; i++) {
      if (scales_idx_x * 128 + i * 64 + lane_id >= dim_x) {
        break;
...
template <typename InputType, typename OutputType, typename ScaleType = float>
__global__ void scale_128x128_kernel(OutputType* output, ScaleType* scales,
                                     InputType const* const input, int dim_x, int dim_y) {
#if (defined(__CUDA_ARCH__) && (__CUDA_ARCH__ >= 900))
...
__global__ void fill_kernel(OutputType* output, size_t num_elems, float value) {
  for (int idx = blockIdx.x * blockDim.x + threadIdx.x; idx < num_elems;
...
void fp8_1x128_cs(__nv_fp8_e4m3* mat_quant, float* scales, __nv_bfloat16 const* mat, int shape_x,
  ...
  scale_1x128_kernel<<<kNumDeviceSMs * 8, 256, 0, stream>>>(mat_quant, scales, mat, shape_x,
                                                            shape_y);
}

void fp8_1x128_cs_reshape(__nv_fp8_e4m3* mat_quant, float* scales, __nv_bfloat16 const* mat,
                          int shape_x, int shape_h, int shape_y, int stride_x,
                          cudaStream_t stream) {
  if (kNumDeviceSMs < 0) {
    kNumDeviceSMs = tensorrt_llm::common::getMultiProcessorCount();
  }
  scale_1x128_reshape_kernel<<<kNumDeviceSMs * 8, 256, 0, stream>>>(mat_quant, scales, mat, shape_x,
                                                                    shape_h, shape_y, stride_x);
}

void fp8_128x128_cs(__nv_fp8_e4m3* mat_quant, float* scales, __nv_bfloat16 const* mat, int shape_x,
                    int shape_y, cudaStream_t stream) {
  if (kNumDeviceSMs < 0) {
    kNumDeviceSMs = tensorrt_llm::common::getMultiProcessorCount();
  }
  convert_kernel<<<kNumDeviceSMs, 256, 0, stream>>>(mat_quant, mat, shape_x * shape_y);
  fill_kernel<<<kNumDeviceSMs, 256, 0, stream>>>(scales,
                                                 div_up(shape_x, 128) * div_up(shape_y, 128), 1);
}

void gemm_dispatch_old(void* mat_a, int ld_a, void* mat_b, int ld_b, void* mat_d, int ld_d,
                       float* scales_a, float* scales_b, int shape_m, int shape_n, int shape_k,
                       cudaStream_t stream) {
  if (kNumDeviceSMs < 0) {
    kNumDeviceSMs = tensorrt_llm::common::getMultiProcessorCount();
  }
...
#define DISPATCH_BLOCK_SIZE(TILE_M, TILE_N)                                                      \
  {                                                                                              \
    using GemmType =                                                                             \
        Fp8Gemm<__nv_fp8_e4m3, Layout::RowMajor, __nv_fp8_e4m3, Layout::ColMajor, __nv_bfloat16, \
                Layout::RowMajor, float, float, float, TILE_M, TILE_N, 128,                      \
                ScaleType::PerSubChannel, ScaleType::PerBlock, 1, 128, 128, 128>;                \
    GemmType::run(reinterpret_cast<__nv_fp8_e4m3*>(mat_a), ld_a,                                 \
                  reinterpret_cast<__nv_fp8_e4m3*>(mat_b), ld_b,                                 \
                  reinterpret_cast<__nv_bfloat16*>(mat_d), ld_d, scales_a, scales_b, shape_m,    \
                  shape_n, shape_k, stream                                                       \
                                                                                                 \
    );                                                                                           \
  }                                                                                              \
  break
...
void gemm_dispatch_old(void* mat_a, void* mat_b, void* mat_d, float* scales_a, float* scales_b,
                       int num_problems, int64_t const* problem_m_offsets, int max_shape_m,
  ...
  {                                                                                              \
    using GemmType =                                                                             \
        Fp8Gemm<__nv_fp8_e4m3, Layout::RowMajor, __nv_fp8_e4m3, Layout::ColMajor, __nv_bfloat16, \
                Layout::RowMajor, float, float, float, TILE_M, TILE_N, 128,                      \
                ScaleType::PerSubChannel, ScaleType::PerBlock, 1, 128, 128, 128>;                \
    GemmType::run(reinterpret_cast<__nv_fp8_e4m3*>(mat_a),                                       \
                  reinterpret_cast<__nv_fp8_e4m3*>(mat_b),                                       \
                  reinterpret_cast<__nv_bfloat16*>(mat_d), scales_a, scales_b, num_problems,     \
                  problem_m_offsets, shape_n, shape_k, max_shape_m, stream                       \
                                                                                                 \
    );                                                                                           \
  }                                                                                              \
  break
...
void gemm_dispatch_sm89(void* mat_a, void* mat_b, void* mat_d, float* scales_a, float* scales_b,
                        uint32_t shape_m, uint32_t shape_n, uint32_t shape_k, cudaStream_t stream,
  ...
  using KT = ada_blockwise_gemm::AdaBlockwiseGemmTraits<ElementInput, ElementOutput, ElementAccum,
                                                        ElementBlockScale, Stages, TileShape::kM,
                                                        TileShape::kN, TileShape::kK>;
  ...
  if (kSmemSize > (48 << 10)) {
    ...
    TLLM_CHECK_WITH_INFO(result == cudaSuccess, "sm89 gemm kernel cannot launch: %s",
                         cudaGetErrorString(result));
  }

  ada_blockwise_gemm::sm89_fp8_gemm_1d1d_impl<GemmKernel><<<grid, block, kSmemSize, stream>>>(
      shape_m, shape_n, shape_k, mat_a, mat_b, mat_d, scales_a, scales_b);
}

void fp8_gemm_run(__nv_fp8_e4m3* mat_a, int ld_a, __nv_fp8_e4m3* mat_b, int ld_b,
                  __nv_bfloat16* mat_d, int ld_d, uint32_t shape_m, uint32_t shape_n,
                  uint32_t shape_k, float* scales_a, float* scales_b, cudaStream_t stream) {
  if (shape_m == 0) {
    return;
  }
...
void fp8_gemm_run(__nv_bfloat16 const* mat_a, __nv_fp8_e4m3* fp8_mat_a, int ld_a, float* scales_a,
                  __nv_bfloat16 const* mat_b, __nv_fp8_e4m3* fp8_mat_b, int ld_b, float* scales_b,
                  __nv_bfloat16* mat_d, int ld_d, uint32_t shape_m, uint32_t shape_n,
                  uint32_t shape_k, cudaStream_t stream, bool internal_quantize_a = true,
  ...
  if (internal_quantize_a) {
    scale_1x128_kernel<<<kNumDeviceSMs * 8, 256, 0, stream>>>(fp8_mat_a, scales_a, mat_a, shape_k,
                                                              shape_m);
  }
  if (internal_quantize_b) {
    scale_128x128_kernel<<<kNumDeviceSMs, 256, 0, stream>>>(fp8_mat_b, scales_b, mat_b, shape_k,
                                                            shape_n);
  }
  fp8_gemm_run(fp8_mat_a, ld_a, fp8_mat_b, ld_b, mat_d, ld_d, shape_m, shape_n, shape_k, scales_a,
               scales_b, stream);
}

void grouped_gemm_dispatch(__nv_fp8_e4m3* mat_a, __nv_fp8_e4m3* mat_b, __nv_bfloat16* mat_d,
                           uint32_t num_problems, int64_t const* problem_m_offsets,
                           uint32_t expected_m, uint32_t max_shape_m, uint32_t max_shape_m_padded,
                           uint32_t shape_n, uint32_t shape_k, float* scales_a, float* scales_b,
  ...
  constexpr uint32_t block_k = 128;
  uint32_t m_per_expert_threshold =
      num_device_sms == 78 ? 64 : 32;  // 64 for H20(sms=78), 32 for H100/H200
  if (expected_m >= m_per_expert_threshold) {
    auto [best_block_m, best_block_n, best_num_stages, best_num_tma_multicast, best_smem_size] =
        deep_gemm::jit::get_best_gemm_config(expected_m, shape_n, shape_k, num_problems,
                                             num_device_sms);

    auto runtime = deep_gemm::jit::getGlobalCompiler().build(
        shape_n, shape_k, best_block_m, best_block_n, block_k, num_problems, best_num_stages,
        best_num_tma_multicast, deep_gemm::GemmType::GroupedWithOffset);
    auto kernel = reinterpret_cast<cudaKernel_t>(runtime->getKernel());
    deep_gemm::runGemm(kernel, mat_a, 0, mat_b, 0, mat_d, 0, scales_a, scales_b, max_shape_m,
                       shape_n, shape_k, best_block_m, best_block_n, block_k, num_problems,
                       best_num_tma_multicast, deep_gemm::GemmType::GroupedWithOffset,
                       const_cast<int64_t*>(problem_m_offsets), stream, num_device_sms,
                       static_cast<uint32_t>(best_smem_size), max_shape_m_padded);
  } else {
    auto [best_block_m, best_block_n, best_num_stages, best_num_tma_multicast, best_smem_size] =
        deep_gemm::jit::get_best_gemm_config(shape_n, expected_m, shape_k, num_problems,
                                             num_device_sms, false, true);
    auto runtime = deep_gemm::jit::getGlobalCompiler().build(
        shape_n, shape_k, best_block_m, best_block_n, block_k, num_problems, best_num_stages,
        best_num_tma_multicast, deep_gemm::GemmType::GroupedWithOffset, true);
    auto kernel = reinterpret_cast<cudaKernel_t>(runtime->getKernel());

    deep_gemm::runGemmSwapAB(
        kernel, mat_b, 0, mat_a, 0, mat_d, 0, scales_b, scales_a, shape_n, max_shape_m, shape_k,
        best_block_m, best_block_n, block_k, num_problems, best_num_tma_multicast,
        deep_gemm::GemmType::GroupedWithOffset, const_cast<int64_t*>(problem_m_offsets), stream,
        num_device_sms, static_cast<uint32_t>(best_smem_size), max_shape_m_padded);
  }
}
...
void fp8_grouped_gemm_run(__nv_bfloat16 const* mat_a, __nv_fp8_e4m3* fp8_mat_a, float* scales_a,
  ...
  if (internal_quantize_b) {
    ...
    for (int i = 0; i < num_problems; i++) {
      scale_128x128_kernel<<<kNumDeviceSMs, 256, 0, stream>>>(fp8_mat_b_tmp, scales_b_tmp,
                                                              mat_b_tmp, shape_k, shape_n);
...
void strided_batch_gemm_dispatch(__nv_fp8_e4m3* mat_a, int ld_a, int stride_a, __nv_fp8_e4m3* mat_b,
                                 int ld_b, int stride_b, __nv_bfloat16* mat_d, int ld_d,
                                 int stride_d, float* scales_a, float* scales_b,
                                 uint32_t num_problems, uint32_t shape_m, uint32_t shape_n,
                                 uint32_t shape_k, cudaStream_t stream,
                                 int num_device_sms = kNumDeviceSMs) {
  if (num_device_sms < 0) {
    num_device_sms = kNumDeviceSMs = tensorrt_llm::common::getMultiProcessorCount();
  }
  ...
  auto runtime = deep_gemm::jit::getGlobalCompiler().build(
      shape_n, shape_k, best_block_m, best_block_n, block_k, num_problems, best_num_stages,
      best_num_tma_multicast, deep_gemm::GemmType::StridedBatched);
  auto kernel = reinterpret_cast<cudaKernel_t>(runtime->getKernel());
  deep_gemm::runGemm(kernel, mat_a, static_cast<uint64_t>(ld_a), static_cast<uint64_t>(stride_a),
                     mat_b, static_cast<uint64_t>(ld_b), static_cast<uint64_t>(stride_b), mat_d,
                     static_cast<uint64_t>(ld_d), static_cast<uint64_t>(stride_d), scales_a,
                     scales_b, shape_m, shape_n, shape_k, best_block_m, best_block_n, block_k,
                     num_problems, best_num_tma_multicast, deep_gemm::GemmType::StridedBatched,
                     stream, num_device_sms, static_cast<uint32_t>(best_smem_size));
}

void strided_batch_gemm_dispatch_sm89(__nv_fp8_e4m3* mat_a, int ld_a, int stride_a,
                                      __nv_fp8_e4m3* mat_b, int ld_b, int stride_b,
                                      __nv_bfloat16* mat_d, int ld_d, int stride_d, float* scales_a,
                                      int stride_scales_a, float* scales_b, uint32_t num_problems,
  ...
  ada_blockwise_gemm::sm89_fp8_bmm_1d1d_impl<GemmKernel><<<grid, block, kSmemSize, stream>>>(
      shape_m, shape_n, shape_k, mat_a, mat_b, mat_d, scales_a, scales_b, stride_a, stride_b,
      stride_d, stride_scales_a, stride_scales_b);
}

void fp8_stride_batch_gemm_run(__nv_bfloat16 const* mat_a, __nv_fp8_e4m3* fp8_mat_a,
                               float* scales_a, int ld_a, int stride_a, int stride_scales_a,
                               __nv_bfloat16 const* mat_b, __nv_fp8_e4m3* fp8_mat_b,
                               float* scales_b, int ld_b, int stride_b, __nv_bfloat16* mat_d,
                               int ld_d, int stride_d, uint32_t num_problems, uint32_t shape_m,
                               uint32_t shape_n, uint32_t shape_k, cudaStream_t stream,
                               bool internal_quantize_a = true, bool internal_quantize_b = true) {
  if (shape_m == 0) {
    return;
  }
  ...
  if (internal_quantize_a) {
    scale_1x128_kernel<<<kNumDeviceSMs * 8, 256, 0, stream>>>(fp8_mat_a, scales_a, mat_a, shape_k,
                                                              shape_m * num_problems);
  }
  if (internal_quantize_b) {
    scale_128x128_kernel<<<kNumDeviceSMs, 256, 0, stream>>>(fp8_mat_b, scales_b, mat_b, shape_k,
                                                            shape_n * num_problems);
  }

  int arch = tensorrt_llm::common::getSMVersion();
  if (arch == 89) {
    strided_batch_gemm_dispatch_sm89(fp8_mat_a, ld_a, stride_a, fp8_mat_b, ld_b, stride_b, mat_d,
                                     ld_d, stride_d, scales_a, stride_scales_a, scales_b,
  ...
  if (getDeepGemmEnabled()) {
    ...
    using GemmType = Fp8Gemm<__nv_fp8_e4m3, Layout::RowMajor, __nv_fp8_e4m3, Layout::ColMajor,
                             __nv_bfloat16, Layout::RowMajor, float, float, float, 128, 64, 128,
                             ScaleType::PerSubChannel, ScaleType::PerBlock, 1, 128, 128, 128>;
    GemmType::run(fp8_mat_a, ld_a, stride_a, fp8_mat_b, ld_b, stride_b, mat_d, ld_d, stride_d,
                  scales_a, stride_scales_a, scales_b, static_cast<int>(shape_m),
                  static_cast<int>(shape_n), static_cast<int>(shape_k),
