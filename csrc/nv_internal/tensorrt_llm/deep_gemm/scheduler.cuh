...
struct NormalScheduler {
  ...
  __device__ __forceinline__ uint32_t get_global_n_idx(uint32_t const shape_dim,
  ...
  __device__ __forceinline__ uint32_t get_global_scales_b_idx(uint32_t const shape_dim,
                                                              uint32_t const block_size,
                                                              uint32_t const& block_idx,
                                                              uint32_t const& m_block_idx = 0) {
    return block_idx * block_size;
...
__host__ __device__ __forceinline__ T_offset compute_padded_offset(T_offset offset,
                                                                   T_index problem_idx) {
  // This formulation ensures that padded_offset[i + 1] - padded_offset[i] >= offset[i + 1] -
  // offset[i].
...
struct SchedulerSelector {
  static constexpr auto select_type() {
    ...
    if constexpr (GT == GemmType::GroupedContiguous)
      return GroupedContiguousScheduler<SHAPE_N, BLOCK_M, BLOCK_N, kNumGroups, kNumTMAMulticast,
                                        kNumNBlocks, kNumNBlocksPerGroup>();
    if constexpr (GT == GemmType::GroupedMasked)
      return GroupedMaskedScheduler<SHAPE_N, SHAPE_K, BLOCK_M, BLOCK_N, BLOCK_K, kNumGroups,
