...
static __device__ __forceinline__ void write_result_to_gmem(
    __nv_bfloat16* gmem_d_this_block, __nv_bfloat16 const* smem_d, uint32_t const m_offset,
    uint32_t const m_boundary, uint32_t const n_offset, uint32_t const shape_n,
    uint32_t const ld_output) {
...
__global__ void __launch_bounds__(
    ...
    fp8_gemm_kernel(__nv_bfloat16* gmem_d, float* scales_b, InputType problem_input,
                    ...
                    __grid_constant__ const CUtensorMap tensor_map_d) {
  ...
  static constexpr uint32_t SMEM_SCALES_B_SIZE =
      ceil_div<uint32_t>(SHAPE_K_SCALES * (kMustUseUniformedScaleB ? 1 : 2) * sizeof(float),
                         sizeof(Barrier)) *
      sizeof(Barrier);
...
#pragma unroll
  for (int i = 0; i < kNumStages; ++i) {
    smem_a[i] =
        reinterpret_cast<__nv_fp8_e4m3*>(smem_buffer + SMEM_D_SIZE + i * SMEM_A_SIZE_PER_STAGE);
    ...
    smem_scales_a[i] = reinterpret_cast<float*>(
        smem_buffer + SMEM_D_SIZE + kNumStages * (SMEM_A_SIZE_PER_STAGE + SMEM_B_SIZE_PER_STAGE) +
...
#pragma unroll
    ...
    while (scheduler.get_next_block(m_block_idx, n_block_idx)) {
      ...
      // NOTES: except the first warp, we want to overlap loading B scales with TMA stores between
      // tasks
      if (threadIdx.x >= 32) {
...
#pragma unroll
      ...
      if constexpr (SchedulerType::gemm_type == GemmType::GroupedWithOffset) {
        ...
        if (!cross_boundary) {
          ...
          write_result_to_gmem<BLOCK_M, BLOCK_N, NUM_WARPS>(
              gmem_d_this_block, smem_d, m_global_idx, scheduler.m_boundary, n_block_idx * BLOCK_N,
              SHAPE_N, SHAPE_N);
        }
...
__global__ void __launch_bounds__(
    ...
    fp8_gemm_kernel_swapAB(
        __nv_bfloat16* gmem_d, float* scales_a, InputType problem_input,
        ...
        const __grid_constant__ CUtensorMap
            tensor_map_scales_b,  // act scales (previously tensor_map_scales_a)
  ...
  static constexpr uint32_t SMEM_SCALES_B_SIZE_PER_STAGE =
      BLOCK_N * sizeof(float);  // B matrix (act) scales
  static constexpr uint32_t SMEM_SCALES_B_SIZE_PER_STAGE_PADDED =
      ceil_div<uint32_t>(BLOCK_N * sizeof(float), 128) *
      128;  // B matrix (act) scales, 128B aligned
  static constexpr uint32_t SHAPE_K_SCALES = ceil_div(SHAPE_K, BLOCK_K);
  static constexpr uint32_t SMEM_SCALES_A_SIZE =
  ...
  if (threadIdx.x == kNumMathThreads) {
    ...
    cute::prefetch_tma_descriptor(reinterpret_cast<cute::TmaDescriptor const*>(&tensor_map_b));
    cute::prefetch_tma_descriptor(
        reinterpret_cast<cute::TmaDescriptor const*>(&tensor_map_scales_b));
    cute::prefetch_tma_descriptor(reinterpret_cast<cute::TmaDescriptor const*>(&tensor_map_d));
  }
  __syncwarp();
...
#pragma unroll
  for (int i = 0; i < kNumStages; ++i) {
    ...
    smem_scales_b[i] = reinterpret_cast<float*>(
        smem_buffer + SMEM_D_SIZE + kNumStages * (SMEM_A_SIZE_PER_STAGE + SMEM_B_SIZE_PER_STAGE) +
        i * SMEM_SCALES_B_SIZE_PER_STAGE_PADDED);
  }
  smem_scales_a =
      reinterpret_cast<float*>(smem_buffer + SMEM_D_SIZE +
                               kNumStages * (SMEM_A_SIZE_PER_STAGE + SMEM_B_SIZE_PER_STAGE +
                                             SMEM_SCALES_B_SIZE_PER_STAGE_PADDED));

  // Fill barriers
  auto barrier_start_ptr =
      reinterpret_cast<Barrier*>(reinterpret_cast<uint8_t*>(smem_scales_a) + SMEM_SCALES_A_SIZE);
#pragma unroll
  for (int i = 0; i < kNumStages; ++i) {
    full_barriers[i] = barrier_start_ptr + i;
    empty_barriers[i] = barrier_start_ptr + kNumStages + i;
...
#pragma unroll
          for (uint32_t s = 0; s < kNumInnerStages; ++s) {
            ...
            if constexpr (SchedulerType::gemm_type == GemmType::GroupedWithOffset) {
              tma_copy<kNumTMAMulticast>(
                  &tensor_map_scales_b, reinterpret_cast<uint64_t*>(&full_barrier),
...
#pragma unroll
        for (int s = 0; s < kNumInnerStages; ++s) {
          ...
          // thread needs to read WGMMA::N / 4 * 2 b scales
...
#pragma unroll
      ...
      if constexpr (SchedulerType::gemm_type == GemmType::GroupedWithOffset) {
        ...
        if (!cross_boundary) {
          // Use TMA store to write back to global memory
          if (threadIdx.x == 0) {
            cute::SM90_TMA_STORE_2D::copy(&tensor_map_d, smem_d, m_block_idx * BLOCK_M,
                                          n_global_idx);
            cute::tma_store_arrive();
            cute::tma_store_wait<0>();
          }
          ...
          constexpr int NUM_WARPS =
              (get_num_threads_per_sm<kNumTMAThreads, kNumMathThreadsPerGroup>(BLOCK_M) - 128) / 32;
          write_result_to_gmem<BLOCK_N, BLOCK_M, NUM_WARPS>(
              gmem_d_this_block, smem_d, n_global_idx, scheduler.n_boundary, m_block_idx * BLOCK_M,
              SHAPE_M, SHAPE_M);
        }
      } else if constexpr (SchedulerType::gemm_type == GemmType::StridedBatched) {
        cutlass::arch::NamedBarrier(kNumMathThreads).sync();
...
__global__ void __launch_bounds__(
    get_num_threads_per_sm<kNumTMAThreads, kNumMathThreadsPerGroup>(BLOCK_M), 1)
    fp8_gemm_kernel_fc1_fused(__nv_fp8_e4m3* gmem_d_fp8, int64_t d_rows, int64_t a_rows,
                              float* gmem_sfa_out, int64_t sfa_out_stride, float* scales_b,
  ...
  DG_STATIC_ASSERT(SchedulerType::gemm_type == GemmType::GroupedWithOffsetFc1Fused,
                   "This kernel only pairs with the FC1-fused scheduler");
  ...
  // Every CTA traps on malformed offsets before any tile math: a host-side
  // check would sync and break graph capture, and the scheduler's
  // signed->unsigned conversion must never wrap into OOB tiles.
  if (threadIdx.x == 0) {
    int64_t prev = __ldg(problem_input.problem_m_offsets);
    bool bad = prev != 0;
#pragma unroll 1
    for (uint32_t g = 1; g <= kNumGroups && !bad; ++g) {
      int64_t cur = __ldg(problem_input.problem_m_offsets + g);
      bad = cur < prev;
      prev = cur;
    }
    if (bad || prev > d_rows || prev > a_rows) {
      printf(
          "fc1_fused: bad offsets (first != 0, decreasing, or offsets[%u]=%lld > capacity "
          "min(D=%lld, A=%lld))\n",
          kNumGroups, static_cast<long long>(prev), static_cast<long long>(d_rows),
          static_cast<long long>(a_rows));
      asm volatile("trap;");
    }
  }
  __syncthreads();  // no thread proceeds past unvalidated offsets
